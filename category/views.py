...
def backfire(category_id, etalon):
    ...
    parents = [{'id':Category.objects.get(number=relative).id,
                'name':etalon.name + ' ' + Category.objects.get(number=relative).number}
               for relative in parray
               if Category.objects.filter(name=etalon.name, number=relative)]
    ...
    for relative in array:
        ...
        elif rdot == edot + 1 and \
                        etalon.number == relative.number[:relative.number.rfind('.')]:
