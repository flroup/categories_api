...
def backfire(category_id, etalon):
    ...
    array = Category.objects.filter(name=etalon.name,
                                    number__startswith=txt)

    parray = ['.'.join(etalon.number.split('.')[0:i+1])
              for i in range(etalon.number.count('.'))][::-1]

    parents = [{'id': Category.objects.get(number=relative).id,
                'name': etalon.name + ' ' +
                Category.objects.get(number=relative).number}
               for relative in parray
               if Category.objects.filter(name=etalon.name,
                                          number=relative)]
    ...
    for relative in array:
        ...
        if rdot == edot and relative.number != etalon.number:
            siblings.append({'id': relative.id,
                             'name': etalon.name + " " +
                             relative.number})

        elif (rdot == edot + 1
              and etalon.number ==
              relative.number[:relative.number.rfind('.')]):
            children.append({'id': relative.id,
                             'name': etalon.name + " " + relative.number})

    return {'id': category_id, 'name': etalon.name + ' ' + etalon.number,
